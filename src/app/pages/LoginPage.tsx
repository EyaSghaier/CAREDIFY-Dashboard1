...
/* ── Floating background ECG (decorative) — pure SVG, no canvas ── */
const BGECGLine: React.FC<{ color: string }> = ({ color }) => {
  const PERIOD  = 260;
  const COPIES  = 10;          // 10×260 = 2600 px — covers any screen width
  ...
  const pts = useMemo(() => {
    // Sample every 2 units for perf (VB_W/2 = 1300 points)
    return Array.from({ length: VB_W / 2 }, (_, i) => {
      const x = i * 2;
      return `${x},${(MID + seg(x)).toFixed(1)}`;
    }).join(' ');
  }, []); // stable — SEG is deterministic

  // Scroll one PERIOD at ~40 units/sec → 6.5 s loop
    ...
    <div className="w-full overflow-hidden relative" style={{ height: VB_H }}>
      ...
      <svg
        style={{
          position: 'absolute',
          top: 0, left: 0,
          width: `${VB_W}px`,   // fixed px — large enough to cover any viewport
          height: `${VB_H}px`,
          animation: `${animId} ${durSec}s linear infinite`,
        }}
...
const i18n = {
  FR: {
    ...
    resetDesc: 'Entrez votre adresse email et nous vous enverrons un lien de réinitialisation.',
    resetBtn: 'Envoyer le lien',
    resetSending: 'Envoi en cours...',
    resetSuccess: 'Lien envoyé ! Vérifiez votre boîte mail.',
  ...
  EN: {
    ...
    resetDesc: 'Enter your email address and we\'ll send you a reset link.',
...
export const LoginPage: React.FC = () => {
  ...
  // ── #1565C0 is the primary blue in light mode ──
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    if (err) {
      setResetMsg({ ok: false, text: err });
    } else {
      setResetMsg({ ok: true, text: t.resetSuccess });
    }
  ...
  const cardShadow  = isDark
    ? '0 32px 80px rgba(18,34,70,0.55), 0 8px 32px rgba(18,34,70,0.35), 0 0 0 1px rgba(18,34,70,0.5)'
    : '0 32px 80px rgba(21,101,192,0.22), 0 8px 32px rgba(21,101,192,0.14), 0 2px 8px rgba(21,101,192,0.1)';
  ...
  const themeColor    = isDark ? '#9CA3AF'  : LIGHT_BLUE;
  const logoGlow      = isDark
    ? '0 0 32px rgba(18,34,70,0.8)'
    : `0 0 24px rgba(21,101,192,0.3)`;
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="absolute bottom-1/4 right-1/4 w-[360px] h-[360px] rounded-full blur-3xl pointer-events-none"
        style={{ background: orb2 }}
      />

      {/* ── Background ECG — top ── */}
      <div className="absolute top-6 left-0 right-0 pointer-events-none">
        <BGECGLine color={bgECGColor} />
      </div>

      {/* ── Background ECG — bottom ── */}
      <div className="absolute bottom-6 left-0 right-0 pointer-events-none opacity-60">
        <BGECGLine color={bgECGColor} />
      </div>
      ...
      <div className="relative z-20 flex justify-between items-center px-6 pt-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            style={{ border: `1px solid ${langBorder}` }}
          >
            {(['FR', 'EN'] as const).map((l) => (
              <button
                ...
                style={{
                  background: lang === l ? langActiveBg : langInactiveBg,
                  color: lang === l ? '#fff' : langInactiveColor,
                }}
              >
                {l}
              </button>
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center px-4 py-8">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          <p
            className="font-bold tracking-[0.35em] text-base"
            style={{ color: accent }}
          >
            CAREDIFY
          </p>
              ...
              <>
                <div className="text-center mb-7">
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto mb-4"
                    style={{ background: isDark ? 'rgba(14,165,233,0.1)' : 'rgba(21,101,192,0.08)', border: `1px solid ${isDark ? 'rgba(14,165,233,0.2)' : 'rgba(21,101,192,0.18)'}` }}>
                    <Mail className="w-5 h-5" style={{ color: accent }} />
                  </div>
                  <h2 className="font-bold text-xl mb-2" style={{ color: textColor }}>{t.resetTitle}</h2>
                  <p className="text-sm" style={{ color: subtitleColor }}>{t.resetDesc}</p>
                </div>
                ...
                <form onSubmit={handleReset} className="space-y-5">
                  ...
                  {resetMsg && (
                    <div className="px-4 py-2.5 rounded-xl text-xs"
                      style={{
                        ...
                        border: `1px solid ${resetMsg.ok ? 'rgba(16,185,129,0.25)' : 'rgba(239,68,68,0.2)'}`,
                  ...
                  <button
                    type="submit"
                    disabled={resetLoading || resetMsg?.ok}
                    ...
                    onMouseEnter={(e) => { if (!resetLoading) e.currentTarget.style.background = btnHover; }}
                    onMouseLeave={(e) => { if (!resetLoading) e.currentTarget.style.background = btnBg; }}
                  >
                    {resetLoading ? <><Loader2 className="w-4 h-4 animate-spin" />{t.resetSending}</> : t.resetBtn}
                  </button>
                </form>

                <button
                  onClick={() => { setShowReset(false); setResetMsg(null); setResetEmail(''); }}
                  className="w-full text-center text-xs mt-5 transition-opacity hover:opacity-75"
                  style={{ color: accent }}
                >
                  {t.resetBack}
                </button>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1.5">
                      ...
                      <a
                        ...
                        style={{ color: accent }}
                        onClick={(e) => { e.preventDefault(); setShowReset(true); setResetEmail(email); }}
